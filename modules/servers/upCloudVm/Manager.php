...
class Manager
{
    ...
    private function callApi(string $method, string $query, array $body = [])
    {
        try
        {
            ...
            {
                case 'get':
                    $response = Request::get("https://api.upcloud.com/1.3".$query);
                    break;
                case 'post':
                    $response = Request::post("https://api.upcloud.com/1.3".$query);
                    break;
                case 'put':
                    $response = Request::put("https://api.upcloud.com/1.3".$query);
                    break;
                case 'delete':
                    $response = Request::delete("https://api.upcloud.com/1.3".$query);
                    break;
            }
        ...
        catch (\Exception $e)
        {
            logModuleCall(
                'upCloudVm',
                strtoupper($method),
                'https://api.upcloud.com/1.3'.$query.PHP_EOL.((!empty($body)) ? json_encode($body, JSON_PRETTY_PRINT) : ""),
                $e->getMessage(),
                $e->getMessage(),
                []
            );
    ...
    public function refreshServer()
    {
        ...
        $results['data']['details']['vnc']        = $details['data']->server->vnc;
    ...
    public function saveVNCConfiguration()
    {
        ...
        $body = [
            'server' => [
                'vnc_password' => filter_input(INPUT_POST, 'vnc_password', FILTER_SANITIZE_STRING)
            ]
        ];
        return $this->callApi('put', '/server/'.$this->server, $body);
    }
    ...
    public function changeVNCStatus()
    {
        ...
        $body = [
            'server' => [
                'vnc' => filter_input(INPUT_POST, 'vnc', FILTER_SANITIZE_STRING)
            ]
        ];
        return $this->callApi('put', '/server/'.$this->server, $body);
    }
